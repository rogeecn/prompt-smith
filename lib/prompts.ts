...
export const buildSystemPrompt = ({
  ...
  return [
    ...
    "- 交互过程中优先使用 single/multi（含可选项）让用户选择，尽量减少 text 填空题；仅当无法给出合理选项时才使用 text。",
    "- 若需要补充细节，优先通过 allow_other 的短输入承接，不要把关键问题全部做成文本填空。",
    ...
    "- reply 中严禁出现字段名：final_prompt、questions、deliberations、is_finished 等。",
    ...
    "- 当需要拒绝并要求明确意图时，reply 必须包含以“例如：”开头的示例性问题。",
